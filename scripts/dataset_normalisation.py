...
# Default minimum word count for text filtering
MIN_WORDS_DEFAULT = 32

replacements = [(r"\s{2,}", " ")]
replacements_re = [(re.compile(repl[0]), repl[1]) for repl in replacements]

meta_data = {
    "MariaNMT_cardiomyopathy.json": {"id_field": "id", "text_field": "text"},
    "MariaNMT_acute_coronary_syndrome.jsonl": {"id_field": "id", "text_field": "text"},
    "MariaNMT_atrial_fibrillation.jsonl": {"id_field": "id", "text_field": "text"},
    "MariaNMT_cardiomyopathy.jsonl": {"id_field": "id", "text_field": "text"},
    "MariaNMT_cardiovascular_disease.jsonl": {"id_field": "id", "text_field": "text"},
    "PubmedPatients_nllb200.json": {"id_field": "patient_uid", "text_field": "text"},
    "meditron_guidelines_gpt4omini.json": {"id_field": "id", "text_field": "text"},
    "DeepL_PUBMED_ABSTRACTS_*.parquet": {"id_field": None, "text_field": "text"},
    ...
    "Scraped.jsonl": {"id_field": None, "text_field": "text"},
    "apollo_guidelines_MariaNMT.json": {"id_field": "id", "text_field": "text"},
    "apollo_guidelines_nllb200.json": {"id_field": "id", "text_field": "text"},
    "apollo_medicalGuideline_en_text_GPT4o_mini.json": {
        "id_field": "id",
        "text_field": "text",
    },
    "GPT4omini_pubmed_cardiomyopathy.json": {"id_field": "id", "text_field": "text"},
    "GeminiFlash15_mimic3_*.jsonl": {"id_field": "id", "text_field": "text"},
    "AGCT_MariaNMT.jsonl": {"id_field": "id", "text_field": "text"},
    "Apollo_medicalGuideline_en_text_GeminiFlash15.json": {
        "id_field": "id",
        "text_field": "text",
    },
    "Apollo_medicalPaper_en_text_*.json": {"id_field": "id", "text_field": "text"},
    "EMEA_*.jsonl": {"id_field": None, "text_field": "text"},
    "MariaNMT_Mimic4_*.json": {"id_field": "id", "text_field": "text"},
    "MariaNMT_mimic3_*.json": {"id_field": "id", "text_field": "text"},
    "meditron_guidelines_gpt4omini.json": {"id_field": "id", "text_field": "text"},
    "ntvg.parquet": {"id_field": None, "text_field": "total_text"},
    "apollo_books_nllb200.jsonl": {"id_field": "id", "text_field": "text"},
    "wikipedia-*.parquet": {"id_field": None, "text_field": "text"},
    "apollo_wiki_mariaNMT.jsonl": {"id_field": "id", "text_field": "text"},
    "GPT4omini_pubmed_*.jsonl": {"id_field": "id", "text_field": "text"},
    "COMBINED_DEID_cleaned.parquet": {"id_field": None, "text_field": "text"},
    "GeminiFlash15_*.jsonl": {"id_field": "id", "text_field": "text"},
    "GeminiFlash15_*.json": {"id_field": "id", "text_field": "text"},
    "GeminiFlash2_*.jsonl": {"id_field": "id", "text_field": "text"},
...
def clean_text(text):
    ...
    text = re.sub(r"(\W{1,8})\1{2,}", r"\1", text)
...
def load_datafiles_from_gcs(gcs_dir, out_dir, separator=None):
    ...
    blobs = [
        blob
        for blob in blobs
        if not any([namer(blob) in _existing_file for _existing_file in existing_files])
    ]
...
def normalise_data(data, file_type, incoming_file_name, min_words=MIN_WORDS_DEFAULT):
        ...
        for k, entry in tqdm.tqdm(enumerate(data)):
            if entry is not None:
                if isinstance(entry[text_field], str) and len(entry[text_field]) > 0:
                    ...
                    if text_word_count < min_words:
                        skipped_count += 1
                        continue
                    cid = (
                        str(entry[id_field])
                        if id_field is not None
                        else f"{incoming_file_name}_{str(k)}"
                    )
                    ...
                    normalised_entry = {
                        ...
                        "approx_token_counts_original": entry.get(
                            "approx_token_counts_original", -1
                        ),
                        "approx_token_counts_translated": entry.get(
                            "approx_token_counts_translated", -1
                        ),
                    }
        ...
        for index, row in tqdm.tqdm(data.iterrows()):
            if isinstance(row[text_field], str) and len(row[text_field]) > 0:
                ...
                cid = (
                    str(row[id_field])
                    if id_field is not None
                    else f"{incoming_file_name}_{str(index)}"
                )
                word_count += text_word_count
                normalised_entry = {
                    "id": cid,
                    ...
                    "approx_token_counts_original": row.get(
                        "approx_token_counts_original", -1
                    ),
                    "approx_token_counts_translated": row.get(
                        "approx_token_counts_translated", -1
                    ),
                }
                normalised_data.append(normalised_entry)
    ...
    if skipped_count > 0:
        print(f"Skipped {skipped_count} entries with fewer than {min_words} words")
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        ...
        help=f"Minimum number of words (space-separated) for a text to be included (default: {MIN_WORDS_DEFAULT})",
    )
